...
function renderLessons() {
  ...
  lessonList.querySelectorAll(".lesson-button").forEach((button) => {
    button.addEventListener("click", () => {
      const lesson = button.dataset.lesson;

      localStorage.setItem("elearning_current_subject", lesson);
      ...
      if (selectedLevel === "SMK") {
        window.location.href = `materi-smk.html?subject=${encodeURIComponent(
          lesson,
        )}&grade=${encodeURIComponent(selectedClass)}`;
      ...
      if (selectedLevel === "SMA") {
        window.location.href = `materi-sma.html?subject=${encodeURIComponent(
          lesson,
        )}&grade=${encodeURIComponent(selectedClass)}`;
